...
globals
	...
	race              	MME_RACE_HUMAN							= ConvertRace(1)
	race              	MME_RACE_ORC							= ConvertRace(2)
	race              	MME_RACE_UNDEAD							= ConvertRace(3)
	race              	MME_RACE_NIGHTELF						= ConvertRace(4)
	race				MME_RACE_NAGA       					= ConvertRace(5)
    race				MME_RACE_DWARVEN    					= ConvertRace(6)
    race				MME_RACE_ICETROLLS  					= ConvertRace(7)
	race				MME_RACE_BLOODELFS  					= ConvertRace(8)
    race				MME_RACE_DARKORC    					= ConvertRace(9)
    race				MME_RACE_FORESTGUARD					= ConvertRace(10)
    race				MME_RACE_FORSAKEN   					= ConvertRace(11)
    race				MME_RACE_GOBLIN     					= ConvertRace(12)
    race				MME_RACE_LIZARDS    					= ConvertRace(13)
    race				MME_RACE_KIRINTOR   					= ConvertRace(14)
    race				MME_RACE_HIGHELF    					= ConvertRace(15)
    race				MME_RACE_GNOLL      					= ConvertRace(16)
	race				MME_RACE_DEMONS      					= ConvertRace(17)
	race				MME_RACE_NERUBIAN       				= ConvertRace(18)
    race				MME_RACE_SATYRS       				    = ConvertRace(19)
    ...
    trigger gg_trg_Chat_Name=null

// User-defined
    location                udg_obnulen_7              = null
    unit                    udg_wq123                  = null
    group                   udg_e21                    = null
    group                   udg_e29                    = null
    ...
    //===================================================
    //NAGA
    trigger gg_trg_Oceanic_Flow                    = null
    trigger gg_trg_Water_Geyser                    = null
    //===================================================
    //Blood Elf
    trigger gg_trg_Phoenix_arrow                   = null
    trigger gg_trg_Veer_Fire_Arrows                = null
    trigger gg_trg_Wave_Of_Fire                    = null
    //===================================================
    //DarkOrc
    trigger gg_trg_Blood_of_Demon                  = null
    trigger gg_trg_Rage_of_War                     = null
    trigger gg_trg_Bloodlust                       = null
    //===================================================
    //Dwarven
	trigger gg_trg_Stunning_Shock                  = null
    trigger gg_trg_Lightning_Storm                 = null
    trigger gg_trg_SniperScope                     = null

    trigger gg_trg_Rune_of_Health                  = null
    trigger gg_trg_Rune_of_Power                   = null
    trigger gg_trg_Rune_of_Speed                   = null
    //===================================================

   
  trigger                 gg_trg_TrigerNerub         = null
    trigger                 gg_trg_TrigerNerub2        = null

        trigger                 gg_trg_svetslepot          = null
    trigger                 gg_trg_samobaff            = null
    trigger                 gg_trg_obnulbaffa          = null
    trigger                 gg_trg_metkadamik          = null
    trigger                 gg_trg_silaStihii          = null
    trigger                 gg_trg_silaStihi2          = null
    trigger                 gg_trg_silaStihi3          = null
    trigger                 gg_trg_prizivelema         = null
    trigger                 gg_trg_arhimagosila        = null
...
//==============================MM Engine====================================
//===========================================================================
function Trig_RealCamera takes nothing returns nothing
    call CameraSetSmoothingFactorBJ( 1000000000.00 )
endfunction

function Trig_StackItems_Func001C takes nothing returns boolean
...
function Trig_StackItems_Conditions takes nothing returns boolean
    if ( not Trig_StackItems_Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_StackItems_Func002Func001C takes nothing returns boolean
...
function Trig_StackItems_Actions takes nothing returns nothing
    ...
    loop
        ...
        if ( Trig_StackItems_Func002Func001C() ) then
...
function InitTrig_StackItems takes nothing returns nothing
	local trigger trg_StackItems
    set trg_StackItems = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( trg_StackItems, EVENT_PLAYER_UNIT_PICKUP_ITEM )
    call TriggerAddCondition( trg_StackItems, Condition( function Trig_StackItems_Conditions ) )
    call TriggerAddAction( trg_StackItems, function Trig_StackItems_Actions )
endfunction
//===========================================================================

function Trig_Camera_Zoom_Actions takes nothing returns nothing
    call SetCameraFieldForPlayer( GetTriggerPlayer(), CAMERA_FIELD_TARGET_DISTANCE, S2R(SubStringBJ(GetEventPlayerChatString(), 7, 49)), 0 )
endfunction
...
function InitTrig_Camera_Zoom takes nothing returns nothing
    ...
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(6), "-zoom", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(7), "-zoom", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(8), "-zoom", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(9), "-zoom", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(10), "-zoom", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Camera_Zoom, Player(11), "-zoom", false )
    call TriggerAddAction( gg_trg_Camera_Zoom, function Trig_Camera_Zoom_Actions )
endfunction

// Чат-команда "-name"
//===========================================================================
function Trig_Chat_Name_Actions takes nothing returns nothing
    //Переменные
    local player p
    local string cmd
    local integer cmdLen
    local string chat
    local integer chatLen
    local string name
    local integer nameLen

    //Аргументы
    set p = GetTriggerPlayer()
    set chat = GetEventPlayerChatString()
    set chatLen = StringLength(chat)

    //Команда
    set cmd = "-name "
    set cmdLen = StringLength(cmd)

    //Введённое имя
    set name = SubString(chat, 6, chatLen)
    set nameLen = StringLength(name)

    //Ограничения на имя
    if (nameLen > 1 and nameLen < 60) then
        //Показать подсказку
        call DisplayTextToForce( GetPlayersAll(), GetPlayerName(p) + " сменил своё имя на ''" + name +"''." )

        //Сменить имя игрока
        call SetPlayerName(p, name)
    endif
endfunction
function InitTrig_Chat_Name takes nothing returns nothing
    set gg_trg_Chat_Name = CreateTrigger(  )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(0), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(1), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(2), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(3), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(4), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(5), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(6), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(7), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(8), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(9), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(10), "-name ", false )
    call TriggerRegisterPlayerChatEvent( gg_trg_Chat_Name, Player(11), "-name ", false )
    call TriggerAddAction( gg_trg_Chat_Name, function Trig_Chat_Name_Actions )
endfunction

// Стартовая камера для всех игроков
//===========================================================================
function Trig_StartedCameraSet_Func001A takes nothing returns nothing
    call SetCameraFieldForPlayer( GetEnumPlayer(), CAMERA_FIELD_TARGET_DISTANCE, 2400.00, 0 )
endfunction

function Trig_StartedCameraSet_Actions takes nothing returns nothing
    call ForForce( GetPlayersAll(), function Trig_StartedCameraSet_Func001A )
endfunction

//===========================================================================
function InitTrig_StartedCameraSet takes nothing returns nothing
    set gg_trg_StartedCameraSet = CreateTrigger(  )
    call TriggerAddAction( gg_trg_StartedCameraSet, function Trig_StartedCameraSet_Actions )
endfunction


//Debugger
//===========================================================================
function Trig_LizardmanRiderAnimationDebug_Conditions takes nothing returns boolean
    if ( not ( GetUnitTypeId(GetAttacker()) == 'o63C' ) ) then
        return false
    endif
    return true
endfunction

function Trig_LizardmanRiderAnimationDebug_Func001C takes nothing returns boolean
...
function Trig_LizardmanRiderAnimationDebug_Actions takes nothing returns nothing
    if ( Trig_LizardmanRiderAnimationDebug_Func001C() ) then
        call SetUnitAnimation( GetAttacker(), "Attack" )
    else
        call SetUnitAnimation( GetAttacker(), "Attack Two" )
    endif
endfunction

//===========================================================================
function InitTrig_LizardmanRiderAnimationDebug takes nothing returns nothing
    set gg_trg_LizardmanRiderAnimationDebug = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_LizardmanRiderAnimationDebug, EVENT_PLAYER_UNIT_ATTACKED )
    call TriggerAddCondition( gg_trg_LizardmanRiderAnimationDebug, Condition( function Trig_LizardmanRiderAnimationDebug_Conditions ) )
    call TriggerAddAction( gg_trg_LizardmanRiderAnimationDebug, function Trig_LizardmanRiderAnimationDebug_Actions )
endfunction
...
//dwarven
//===========================================================================
function Trig_Stunning_Shock_Conditions takes nothing returns boolean
    return GetSpellAbilityId() == 'A60M'
endfunction

function Trig_Stunning_Shock_Actions takes nothing returns nothing
    local unit caster
    local unit dummy
    local location caster_point
    local location target_point
    local integer looping
    set caster = GetSpellAbilityUnit()
    set caster_point = GetUnitLoc(caster)
    set target_point = GetSpellTargetLoc()
    set looping = 1
    loop
        exitwhen looping > 5
        call CreateNUnitsAtLocFacingLocBJ( 1, 'u600', GetOwningPlayer(caster), PolarProjectionBJ(caster_point, ( 150.00 * I2R(looping) ), AngleBetweenPoints(caster_point, target_point)), target_point )
        set dummy = GetLastCreatedUnit()
        call SetUnitAbilityLevelSwapped( 'A600', dummy, GetUnitAbilityLevelSwapped('A60M', caster) )
...
//===========================================================================
function InitTrig_Stunning_Shock takes nothing returns nothing
    set gg_trg_Stunning_Shock = CreateTrigger( )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Stunning_Shock, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Stunning_Shock, Condition( function Trig_Stunning_Shock_Conditions ) )
    call TriggerAddAction( gg_trg_Stunning_Shock, function Trig_Stunning_Shock_Actions )
endfunction
//===========================================================================
// ГРОЗОВАЯ БУРЯ

function Trig_Lightning_Storm_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A60S'
endfunction


function Trig_Lightning_Storm_Func003A takes nothing returns nothing
    local unit caster                       = GetSpellAbilityUnit()
    local location caster_point             = GetUnitLoc(caster)
    local integer handle_id                 = GetHandleId(caster)


    if (  ( IsUnitType(GetEnumUnit(), UNIT_TYPE_STRUCTURE) == false ) and (IsUnitAliveBJ(GetEnumUnit()) == true ) and ( IsUnitEnemy(GetEnumUnit(), GetOwningPlayer(caster)) == true ) and ( IsUnitType(GetEnumUnit(), UNIT_TYPE_MAGIC_IMMUNE) == false ) ) then

        call AddSpecialEffectLocBJ( GetUnitLoc(GetEnumUnit()), "Abilities\\Spells\\Other\\Monsoon\\MonsoonBoltTarget.mdl" )
        call UnitDamageTargetBJ( caster, GetEnumUnit(), 425.00, ATTACK_TYPE_NORMAL, DAMAGE_TYPE_LIGHTNING )
...
function Trig_Lightning_Storm_Actions takes nothing returns nothing
    local unit caster                       = GetSpellAbilityUnit()
    local location caster_point             = GetUnitLoc(caster)
    local integer handle_id                 = GetHandleId(caster)

    call SaveUnitHandle(udg_hash,handle_id,1,caster)

    call ForGroupBJ( GetUnitsInRangeOfLocAll(800.00, caster_point), function Trig_Lightning_Storm_Func003A )
endfunction

//===========================================================================
function InitTrig_Lightning_Storm takes nothing returns nothing
    set gg_trg_Lightning_Storm = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Lightning_Storm, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Lightning_Storm, Condition( function Trig_Lightning_Storm_Conditions ) )
    call TriggerAddAction( gg_trg_Lightning_Storm, function Trig_Lightning_Storm_Actions )
endfunction
//===========================================================================
// ОРУЖЕЙНЫЙ ПРИЦЕЛ 

function Trig_SniperScope_Conditions takes nothing returns boolean

    return GetLearnedSkillBJ() == 'A60W'
endfunction


function Trig_SniperScope_Actions takes nothing returns nothing
    local unit caster = GetLearningUnit()
    local integer spell_level = GetUnitAbilityLevelSwapped('A60W', caster)
    local player player_caster = GetOwningPlayer(caster)

    local integer tech = 'R60J'
    if ( spell_level == 1 ) then
        call SetPlayerTechResearchedSwap( tech, 1, player_caster )
    else
        if ( spell_level == 2 ) then
            call SetPlayerTechResearchedSwap( tech, 2, player_caster )
        else
            if ( spell_level == 3 ) then
                call SetPlayerTechResearchedSwap( tech, 3, player_caster )
            else
                call DoNothing(  )
            endif
        endif
...
//===========================================================================
function InitTrig_SniperScope takes nothing returns nothing
    set gg_trg_SniperScope = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_SniperScope, EVENT_PLAYER_HERO_SKILL )
    call TriggerAddCondition( gg_trg_SniperScope, Condition( function Trig_SniperScope_Conditions ) )
    call TriggerAddAction( gg_trg_SniperScope, function Trig_SniperScope_Actions )
endfunction
...
//===========================================================================
//
//  RUNECASTER ABILITIES
//
//===========================================================================
//  RUNE OF HEALTH
function Trig_Rune_of_Health_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A61B'
endfunction


function Trig_Rune_of_Health_Actions takes nothing returns nothing
    local unit caster                   = GetSpellAbilityUnit()
    local unit target                   = GetSpellTargetUnit()
    local location target_point         = GetUnitLoc(target)
    local player caster_player          = GetOwningPlayer(caster)
    local unit dummy_unit               = null

    call CreateNUnitsAtLoc( 1, 'e60H', caster_player, target_point, bj_UNIT_FACING )
    set dummy_unit = GetLastCreatedUnit()

    if ( IsUnitEnemy(target, caster_player) == true ) then
        
        call UnitAddAbilityBJ( 'A61F', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "unholyfrenzy", target )
    else

        call UnitAddAbilityBJ( 'A61E', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "rejuvination", target )
    endif

    call RemoveUnit( dummy_unit )
    call RemoveLocation (target_point)
    
    set dummy_unit = null
    set target = null
    set caster = null
    set caster_player = null
endfunction

function InitTrig_Rune_of_Health takes nothing returns nothing
    set gg_trg_Rune_of_Health = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Rune_of_Health, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Rune_of_Health, Condition( function Trig_Rune_of_Health_Conditions ) )
    call TriggerAddAction( gg_trg_Rune_of_Health, function Trig_Rune_of_Health_Actions )
endfunction

//===========================================================================
//  RUNE OF POWER

function Trig_Rune_of_Power_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A61C'
endfunction


function Trig_Rune_of_Power_Actions takes nothing returns nothing
    local unit caster                   = GetSpellAbilityUnit()
    local unit target                   = GetSpellTargetUnit()
    local location target_point         = GetUnitLoc(target)
    local player caster_player          = GetOwningPlayer(caster)
    ...
    if ( IsUnitEnemy(target, caster_player) == true ) then
        ...
        call UnitAddAbilityBJ( 'S602', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "unholyfrenzy", target )
    else

        call UnitAddAbilityBJ( 'A61G', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "rejuvination", target )
    endif
...
function InitTrig_Rune_of_Power takes nothing returns nothing
    set gg_trg_Rune_of_Power = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Rune_of_Power, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Rune_of_Power, Condition( function Trig_Rune_of_Power_Conditions ) )
    call TriggerAddAction( gg_trg_Rune_of_Power, function Trig_Rune_of_Power_Actions )
endfunction

//===========================================================================
//  RUNE OF SPEED

function Trig_Rune_of_Speed_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A61D'
endfunction

function Trig_Rune_of_Speed_Actions takes nothing returns nothing
    local unit caster                   = GetSpellAbilityUnit()
    local unit target                   = GetSpellTargetUnit()
    local location target_point         = GetUnitLoc(target)
    local player caster_player          = GetOwningPlayer(caster)
    ...
    if ( IsUnitEnemy(target, caster_player) == true ) then
        ...
        call UnitAddAbilityBJ( 'A61I', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "unholyfrenzy", target )
    else

        call UnitAddAbilityBJ( 'A61H', dummy_unit )
        call IssueTargetOrderBJ( dummy_unit, "rejuvination", target )
    endif
...
function InitTrig_Rune_of_Speed takes nothing returns nothing
    set gg_trg_Rune_of_Speed = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Rune_of_Speed, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Rune_of_Speed, Condition( function Trig_Rune_of_Speed_Conditions ) )
    call TriggerAddAction( gg_trg_Rune_of_Speed, function Trig_Rune_of_Speed_Actions )
endfunction
...
// Trigger: TrigerNerub
//===========================================================================
function Trig_TrigerNerub_Conditions takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('S609', GetTrainedUnit()) > 0 ) ) then
        return false
    endif
    return true
endfunction

function Trig_TrigerNerub_Actions takes nothing returns nothing
    call IssueImmediateOrder( GetTrainedUnit(), "avengerform" )
endfunction

//===========================================================================
function InitTrig_TrigerNerub takes nothing returns nothing
    set gg_trg_TrigerNerub = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_TrigerNerub, EVENT_PLAYER_UNIT_TRAIN_FINISH )
    call TriggerAddCondition( gg_trg_TrigerNerub, Condition( function Trig_TrigerNerub_Conditions ) )
    call TriggerAddAction( gg_trg_TrigerNerub, function Trig_TrigerNerub_Actions )
endfunction

//===========================================================================
// Trigger: TrigerNerub2
//===========================================================================
function Trig_TrigerNerub2_Conditions takes nothing returns boolean
    if ( not ( GetUnitTypeId(GetTrainedUnit()) == 'o64E' ) ) then
        return false
    endif
    if ( not ( GetPlayerController(GetOwningPlayer(GetTrainedUnit())) == MAP_CONTROL_COMPUTER ) ) then
        return false
    endif
    if ( not ( CountLivingPlayerUnitsOfTypeId('o64K', GetOwningPlayer(GetTrainedUnit())) == 0 ) ) then
        return false
    endif
    return true
endfunction

function Trig_TrigerNerub2_Actions takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'o64K', GetOwningPlayer(GetTrainedUnit()), GetPlayerStartLocationLoc(GetOwningPlayer(GetTrainedUnit())), bj_UNIT_FACING )
endfunction

//===========================================================================
function InitTrig_TrigerNerub2 takes nothing returns nothing
    set gg_trg_TrigerNerub2 = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_TrigerNerub2, EVENT_PLAYER_UNIT_TRAIN_FINISH )
    call TriggerAddCondition( gg_trg_TrigerNerub2, Condition( function Trig_TrigerNerub2_Conditions ) )
    call TriggerAddAction( gg_trg_TrigerNerub2, function Trig_TrigerNerub2_Actions )
endfunction
...
// Trigger: svetslepot
//===========================================================================
function Trig_svetslepot_Conditions takes nothing returns boolean
    if ( not ( GetSpellAbilityId() == 'A6NH' ) ) then
        return false
    endif
    return true
endfunction

function Trig_svetslepot_Func007C takes nothing returns boolean
    if ( not ( IsUnitType(GetSpellTargetUnit(), UNIT_TYPE_UNDEAD) == true ) ) then
        return false
    endif
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 0 ) ) then
        return false
    endif
    return true
endfunction

function Trig_svetslepot_Func008C takes nothing returns boolean
    if ( not ( IsUnitType(GetSpellTargetUnit(), UNIT_TYPE_UNDEAD) == true ) ) then
        return false
    endif
...
function Trig_svetslepot_Func009C takes nothing returns boolean
    if ( not ( IsUnitType(GetSpellTargetUnit(), UNIT_TYPE_UNDEAD) == true ) ) then
        return false
    endif
...
function Trig_svetslepot_Actions takes nothing returns nothing
    set udg_obnulen_7 = GetUnitLoc(GetSpellTargetUnit())
    call CreateNUnitsAtLoc( 1, 'e61C', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call IssueTargetOrder( GetLastCreatedUnit(), "curse", GetSpellTargetUnit() )
    call RemoveLocation( udg_obnulen_7)
    if ( Trig_svetslepot_Func007C() ) then
        call SetUnitLifeBJ( GetSpellTargetUnit(), ( GetUnitStateSwap(UNIT_STATE_LIFE, GetSpellTargetUnit()) - 25.00 ) )
    else
    endif
    if ( Trig_svetslepot_Func008C() ) then
        call SetUnitLifeBJ( GetSpellTargetUnit(), ( GetUnitStateSwap(UNIT_STATE_LIFE, GetSpellTargetUnit()) - 50.00 ) )
    else
    endif
    if ( Trig_svetslepot_Func009C() ) then
        call SetUnitLifeBJ( GetSpellTargetUnit(), ( GetUnitStateSwap(UNIT_STATE_LIFE, GetSpellTargetUnit()) - 75.00 ) )
    else
    endif
endfunction

//===========================================================================
function InitTrig_svetslepot takes nothing returns nothing
    set gg_trg_svetslepot = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_svetslepot, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_svetslepot, Condition( function Trig_svetslepot_Conditions ) )
    call TriggerAddAction( gg_trg_svetslepot, function Trig_svetslepot_Actions )
endfunction

//===========================================================================
// Trigger: samobaff
//===========================================================================
function Trig_samobaff_Conditions takes nothing returns boolean
    if ( not ( GetLearnedSkillBJ() == 'A6NJ' ) ) then
        return false
    endif
    return true
endfunction

function Trig_samobaff_Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NJ', GetLearningUnit()) == 1 ) ) then
        return false
    endif
    return true
endfunction

function Trig_samobaff_Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NJ', GetLearningUnit()) == 2 ) ) then
        return false
    endif
    return true
endfunction

function Trig_samobaff_Func003C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NJ', GetLearningUnit()) == 3 ) ) then
        return false
    endif
    return true
endfunction

function Trig_samobaff_Actions takes nothing returns nothing
    if ( Trig_samobaff_Func001C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetTriggerUnit() )
    else
    endif
    if ( Trig_samobaff_Func002C() ) then
        call UnitAddAbilityBJ( 'A6NL', GetTriggerUnit() )
    else
    endif
    if ( Trig_samobaff_Func003C() ) then
        call UnitAddAbilityBJ( 'A6NM', GetTriggerUnit() )
    else
    endif
endfunction

//===========================================================================
function InitTrig_samobaff takes nothing returns nothing
    set gg_trg_samobaff = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_samobaff, EVENT_PLAYER_HERO_SKILL )
    call TriggerAddCondition( gg_trg_samobaff, Condition( function Trig_samobaff_Conditions ) )
    call TriggerAddAction( gg_trg_samobaff, function Trig_samobaff_Actions )
endfunction

//===========================================================================
// Trigger: obnulbaffa
//===========================================================================
function Trig_obnulbaffa_Conditions takes nothing returns boolean
    if ( not ( GetItemTypeId(GetManipulatedItem()) == 'tret' ) ) then
        return false
    endif
    if ( not ( GetUnitAbilityLevelSwapped('A6NK', GetManipulatingUnit()) == 1 ) ) then
        return false
    endif
    return true
endfunction

function Trig_obnulbaffa_Actions takes nothing returns nothing
    call UnitRemoveAbilityBJ( 'A6NK', GetManipulatingUnit() )
    call UnitRemoveAbilityBJ( 'A6NL', GetManipulatingUnit() )
    call UnitRemoveAbilityBJ( 'A6NM', GetManipulatingUnit() )
endfunction

//===========================================================================
function InitTrig_obnulbaffa takes nothing returns nothing
    set gg_trg_obnulbaffa = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_obnulbaffa, EVENT_PLAYER_UNIT_USE_ITEM )
    call TriggerAddCondition( gg_trg_obnulbaffa, Condition( function Trig_obnulbaffa_Conditions ) )
    call TriggerAddAction( gg_trg_obnulbaffa, function Trig_obnulbaffa_Actions )
endfunction

//===========================================================================
// Trigger: metkadamik
//===========================================================================
function Trig_metkadamik_Conditions takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('S60A', GetDyingUnit()) == 1 ) ) then
        return false
    endif
    return true
endfunction

function Trig_metkadamik_Actions takes nothing returns nothing
    call RemoveUnit( GetDyingUnit() )
endfunction

//===========================================================================
function InitTrig_metkadamik takes nothing returns nothing
    set gg_trg_metkadamik = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_metkadamik, EVENT_PLAYER_UNIT_DEATH )
    call TriggerAddCondition( gg_trg_metkadamik, Condition( function Trig_metkadamik_Conditions ) )
    call TriggerAddAction( gg_trg_metkadamik, function Trig_metkadamik_Actions )
endfunction

//===========================================================================
// Trigger: silaStihii
//===========================================================================
function Trig_silaStihii_Conditions takes nothing returns boolean
    if ( not ( GetSpellAbilityId() == 'A6NO' ) ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihii_Func001Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NN', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "firebolt", udg_wq123 )
endfunction

function Trig_silaStihii_Func001Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(3, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihii_Func001Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihii_Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 1 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihii_Func002Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NN', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "firebolt", udg_wq123 )
endfunction

function Trig_silaStihii_Func002Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(6, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihii_Func002Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihii_Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 2 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihii_Func003Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NN', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "thunderbolt", udg_wq123 )
endfunction

function Trig_silaStihii_Func003Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(9, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihii_Func003Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihii_Func003C takes nothing returns boolean
...
function Trig_silaStihii_Actions takes nothing returns nothing
    if ( Trig_silaStihii_Func001C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n611')
        call ForGroupBJ( udg_e21, function Trig_silaStihii_Func001Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihii_Func002C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n611')
        call ForGroupBJ( udg_e21, function Trig_silaStihii_Func002Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihii_Func003C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n611')
        call ForGroupBJ( udg_e21, function Trig_silaStihii_Func003Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
...
//===========================================================================
function InitTrig_silaStihii takes nothing returns nothing
    set gg_trg_silaStihii = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_silaStihii, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_silaStihii, Condition( function Trig_silaStihii_Conditions ) )
    call TriggerAddAction( gg_trg_silaStihii, function Trig_silaStihii_Actions )
endfunction

//===========================================================================
// Trigger: silaStihi2
//===========================================================================
function Trig_silaStihi2_Func001C takes nothing returns boolean
    if ( ( GetSpellAbilityId() == 'A6NP' ) ) then
        return true
    endif
    if ( ( GetSpellAbilityId() == 'A6NQ' ) ) then
        return true
    endif
    if ( ( GetSpellAbilityId() == 'A6NR' ) ) then
        return true
    endif
    if ( ( GetSpellAbilityId() == 'A6NS' ) ) then
        return true
    endif
    return false
endfunction

function Trig_silaStihi2_Conditions takes nothing returns boolean
    if ( not Trig_silaStihi2_Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi2_Func002Func001C takes nothing returns boolean
...
function Trig_silaStihi2_Func002Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NT', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "shadowstrike", udg_wq123 )
endfunction

function Trig_silaStihi2_Func002Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(3, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi2_Func002Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi2_Func002C takes nothing returns boolean
    if ( not Trig_silaStihi2_Func002Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi2_Func003Func001C takes nothing returns boolean
...
function Trig_silaStihi2_Func003Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NT', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "shadowstrike", udg_wq123 )
endfunction

function Trig_silaStihi2_Func003Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(5, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi2_Func003Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi2_Func003C takes nothing returns boolean
    if ( not Trig_silaStihi2_Func003Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi2_Func004Func001C takes nothing returns boolean
...
function Trig_silaStihi2_Func004Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NT', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "shadowstrike", udg_wq123 )
endfunction

function Trig_silaStihi2_Func004Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(7, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi2_Func004Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi2_Func004C takes nothing returns boolean
    if ( not Trig_silaStihi2_Func004Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi2_Actions takes nothing returns nothing
    if ( Trig_silaStihi2_Func002C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62B')
        call ForGroupBJ( udg_e21, function Trig_silaStihi2_Func002Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi2_Func003C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62B')
        call ForGroupBJ( udg_e21, function Trig_silaStihi2_Func003Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi2_Func004C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62B')
        call ForGroupBJ( udg_e21, function Trig_silaStihi2_Func004Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
...
//===========================================================================
function InitTrig_silaStihi2 takes nothing returns nothing
    set gg_trg_silaStihi2 = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_silaStihi2, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_silaStihi2, Condition( function Trig_silaStihi2_Conditions ) )
    call TriggerAddAction( gg_trg_silaStihi2, function Trig_silaStihi2_Actions )
endfunction

//===========================================================================
// Trigger: silaStihi3
//===========================================================================
function Trig_silaStihi3_Conditions takes nothing returns boolean
    if ( not ( GetSpellAbilityId() == 'A6NV' ) ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi3_Func001Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NU', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssuePointOrderLoc( GetLastCreatedUnit(), "silence", udg_obnulen_7 )
endfunction

function Trig_silaStihi3_Func001Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(3, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi3_Func001Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi3_Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 1 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi3_Func002Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NU', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssuePointOrderLoc( GetLastCreatedUnit(), "silence", udg_obnulen_7 )
endfunction

function Trig_silaStihi3_Func002Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(6, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi3_Func002Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi3_Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 2 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi3_Func003Func005Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6NU', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssuePointOrderLoc( GetLastCreatedUnit(), "silence", udg_obnulen_7 )
endfunction

function Trig_silaStihi3_Func003Func005A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(9, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi3_Func003Func005Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi3_Func003C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 3 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi3_Actions takes nothing returns nothing
    if ( Trig_silaStihi3_Func001C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(udg_wq123)
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62A')
        call ForGroupBJ( udg_e21, function Trig_silaStihi3_Func001Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi3_Func002C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(udg_wq123)
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62A')
        call ForGroupBJ( udg_e21, function Trig_silaStihi3_Func002Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi3_Func003C() ) then
        set udg_wq123 = GetSpellTargetUnit()
        set udg_obnulen_7 = GetUnitLoc(udg_wq123)
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62A')
        call ForGroupBJ( udg_e21, function Trig_silaStihi3_Func003Func005A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
...
//===========================================================================
function InitTrig_silaStihi3 takes nothing returns nothing
    set gg_trg_silaStihi3 = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_silaStihi3, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_silaStihi3, Condition( function Trig_silaStihi3_Conditions ) )
    call TriggerAddAction( gg_trg_silaStihi3, function Trig_silaStihi3_Actions )
endfunction

//===========================================================================
// Trigger: prizivelema
//===========================================================================
function Trig_prizivelema_Conditions takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6O2', GetSummonedUnit()) > 0 ) ) then
        return false
    endif
    return true
endfunction

function Trig_prizivelema_Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 1 ) ) then
        return false
    endif
    return true
endfunction

function Trig_prizivelema_Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 2 ) ) then
        return false
    endif
    return true
endfunction

function Trig_prizivelema_Func003C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 3 ) ) then
        return false
    endif
    return true
endfunction

function Trig_prizivelema_Actions takes nothing returns nothing
    if ( Trig_prizivelema_Func001C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetSummonedUnit() )
    else
    endif
    if ( Trig_prizivelema_Func002C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NL', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NZ', GetSummonedUnit() )
    else
    endif
    if ( Trig_prizivelema_Func003C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NL', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NM', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6NZ', GetSummonedUnit() )
        call UnitAddAbilityBJ( 'A6O0', GetSummonedUnit() )
    else
    endif
endfunction

//===========================================================================
function InitTrig_prizivelema takes nothing returns nothing
    set gg_trg_prizivelema = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_prizivelema, EVENT_PLAYER_UNIT_SUMMON )
    call TriggerAddCondition( gg_trg_prizivelema, Condition( function Trig_prizivelema_Conditions ) )
    call TriggerAddAction( gg_trg_prizivelema, function Trig_prizivelema_Actions )
endfunction

//===========================================================================
// Trigger: arhimagosila
//===========================================================================
function Trig_arhimagosila_Conditions takes nothing returns boolean
    if ( not ( GetSpellAbilityId() == 'A6IP' ) ) then
        return false
    endif
    return true
endfunction

function Trig_arhimagosila_Func001002002 takes nothing returns boolean
    return ( GetUnitAbilityLevelSwapped('A6O2', GetFilterUnit()) > 0 )
endfunction

function Trig_arhimagosila_Func002A takes nothing returns nothing
    set udg_obnulen_7 = GetUnitLoc(GetEnumUnit())
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6O3', GetLastCreatedUnit(), GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) )
    call IssueImmediateOrder( GetLastCreatedUnit(), "thunderclap" )
    call RemoveLocation( udg_obnulen_7)
endfunction

function Trig_arhimagosila_Func003Func001C takes nothing returns boolean
    if ( ( UnitHasBuffBJ(GetSpellTargetUnit(), 'B65E') == true ) ) then
        return true
    endif
    if ( ( UnitHasBuffBJ(GetSpellTargetUnit(), 'B65F') == true ) ) then
        return true
    endif
    return false
endfunction

function Trig_arhimagosila_Func003C takes nothing returns boolean
    if ( not Trig_arhimagosila_Func003Func001C() ) then
        return false
    endif
    return true
endfunction

function Trig_arhimagosila_Actions takes nothing returns nothing
    set udg_e21 = GetUnitsOfPlayerMatching(GetOwningPlayer(GetSpellAbilityUnit()), Condition(function Trig_arhimagosila_Func001002002))
    call ForGroupBJ( udg_e21, function Trig_arhimagosila_Func002A )
    if ( Trig_arhimagosila_Func003C() ) then
        call SetUnitLifeBJ( GetSpellTargetUnit(), ( GetUnitStateSwap(UNIT_STATE_LIFE, GetSpellTargetUnit()) - 55.00 ) )
    else
    endif
    call DestroyGroup(udg_e21)
endfunction
...
//===========================================================================
function InitTrig_arhimagosila takes nothing returns nothing
    set gg_trg_arhimagosila = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_arhimagosila, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_arhimagosila, Condition( function Trig_arhimagosila_Conditions ) )
    call TriggerAddAction( gg_trg_arhimagosila, function Trig_arhimagosila_Actions )
endfunction

//===========================================================================
// Trigger: prokelem
//===========================================================================
function Trig_prokelem_Conditions takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6O2', GetKillingUnitBJ()) > 0 ) ) then
        return false
    endif
    return true
endfunction

function Trig_prokelem_Func001002002 takes nothing returns boolean
    return ( GetUnitAbilityLevelSwapped('A6O4', GetFilterUnit()) > 0 )
endfunction

function Trig_prokelem_Func002A takes nothing returns nothing
...
function Trig_prokelem_Actions takes nothing returns nothing
    set udg_e21 = GetUnitsOfPlayerMatching(GetOwningPlayer(GetKillingUnitBJ()), Condition(function Trig_prokelem_Func001002002))
    call ForGroupBJ( udg_e21, function Trig_prokelem_Func002A )
    call DestroyGroup(udg_e21)
endfunction

//===========================================================================
function InitTrig_prokelem takes nothing returns nothing
    set gg_trg_prokelem = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_prokelem, EVENT_PLAYER_UNIT_DEATH )
    call TriggerAddCondition( gg_trg_prokelem, Condition( function Trig_prokelem_Conditions ) )
    call TriggerAddAction( gg_trg_prokelem, function Trig_prokelem_Actions )
endfunction

//===========================================================================
// Trigger: silaStihi4
//===========================================================================
function Trig_silaStihi4_Conditions takes nothing returns boolean
    if ( not ( GetSpellAbilityId() == 'A6O5' ) ) then
        return false
    endif
    return true
endfunction

function Trig_silaStihi4_Func001Func004Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6O6', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "frostarmor", GetSpellTargetUnit() )
endfunction

function Trig_silaStihi4_Func001Func004A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(3, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi4_Func001Func004Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi4_Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 1 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi4_Func002Func004Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6O6', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "frostarmor", GetSpellTargetUnit() )
endfunction

function Trig_silaStihi4_Func002Func004A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(6, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi4_Func002Func004Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi4_Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 2 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi4_Func003Func004Func002A takes nothing returns nothing
    call CreateNUnitsAtLoc( 1, 'e61E', GetOwningPlayer(GetSpellAbilityUnit()), udg_obnulen_7, bj_UNIT_FACING )
    call SetUnitAbilityLevelSwapped( 'A6O6', GetLastCreatedUnit(), CountUnitsInGroup(udg_e29) )
    call IssueTargetOrder( GetLastCreatedUnit(), "frostarmor", GetSpellTargetUnit() )
endfunction

function Trig_silaStihi4_Func003Func004A takes nothing returns nothing
    set udg_e29 = GetRandomSubGroup(9, udg_e21)
    call ForGroupBJ( udg_e29, function Trig_silaStihi4_Func003Func004Func002A )
    call DestroyGroup(udg_e29)
endfunction

function Trig_silaStihi4_Func003C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped(GetSpellAbilityId(), GetSpellAbilityUnit()) == 3 ) ) then
        return false
    endif
    return true
...
function Trig_silaStihi4_Actions takes nothing returns nothing
    if ( Trig_silaStihi4_Func001C() ) then
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62C')
        call ForGroupBJ( udg_e21, function Trig_silaStihi4_Func001Func004A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi4_Func002C() ) then
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62C')
        call ForGroupBJ( udg_e21, function Trig_silaStihi4_Func002Func004A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
    if ( Trig_silaStihi4_Func003C() ) then
        set udg_obnulen_7 = GetUnitLoc(GetSpellAbilityUnit())
        set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSpellAbilityUnit()), 'n62C')
        call ForGroupBJ( udg_e21, function Trig_silaStihi4_Func003Func004A )
        call RemoveLocation( udg_obnulen_7)
        call DestroyGroup(udg_e21)
    else
    endif
...
//===========================================================================
function InitTrig_silaStihi4 takes nothing returns nothing
    set gg_trg_silaStihi4 = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_silaStihi4, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_silaStihi4, Condition( function Trig_silaStihi4_Conditions ) )
    call TriggerAddAction( gg_trg_silaStihi4, function Trig_silaStihi4_Actions )
endfunction

//===========================================================================
// Trigger: prizivelema2
//===========================================================================
function Trig_prizivelema2_Conditions takes nothing returns boolean
    if ( not ( GetUnitTypeId(GetSummonedUnit()) == 'e61I' ) ) then
        return false
    endif
    return true
endfunction

function Trig_prizivelema2_Func003Func001C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 1 ) ) then
        return false
    endif
    return true
...
function Trig_prizivelema2_Func003Func002C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 2 ) ) then
        return false
    endif
    return true
...
function Trig_prizivelema2_Func003Func003C takes nothing returns boolean
    if ( not ( GetUnitAbilityLevelSwapped('A6NX', GetSummoningUnit()) == 3 ) ) then
        return false
    endif
    return true
...
function Trig_prizivelema2_Func003A takes nothing returns nothing
    if ( Trig_prizivelema2_Func003Func001C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetEnumUnit() )
    else
    endif
    if ( Trig_prizivelema2_Func003Func002C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NL', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NZ', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetEnumUnit() )
    else
    endif
    if ( Trig_prizivelema2_Func003Func003C() ) then
        call UnitAddAbilityBJ( 'A6NK', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NL', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NM', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NY', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6NZ', GetEnumUnit() )
        call UnitAddAbilityBJ( 'A6O0', GetEnumUnit() )
    else
    endif
endfunction

function Trig_prizivelema2_Actions takes nothing returns nothing
    call TriggerSleepAction( 2 )
    set udg_e21 = GetUnitsOfPlayerAndTypeId(GetOwningPlayer(GetSummoningUnit()), 'n65B')
    call ForGroupBJ( udg_e21, function Trig_prizivelema2_Func003A )
    call DestroyGroup(udg_e21)
endfunction

//===========================================================================
function InitTrig_prizivelema2 takes nothing returns nothing
    set gg_trg_prizivelema2 = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_prizivelema2, EVENT_PLAYER_UNIT_SUMMON )
    call TriggerAddCondition( gg_trg_prizivelema2, Condition( function Trig_prizivelema2_Conditions ) )
...
//===========================================================================
//
//  BLOOD ELF HEROES ABILITIES 
//
//===========================================================================
// СТРЕЛА ФЕНИКСА

function Trig_Phoenix_arrow_timer takes nothing returns nothing
    ...
    if ( GetWidgetLife( Dummy ) < 0.405 ) then
        ...
        set  Dummy  =  CreateUnit( GetOwningPlayer( Caster ), 'u60D', xd, yd, 0.0 )
...
function Trig_Phoenix_arrow_Actions takes nothing returns nothing
    ...
    local  unit     Dummy    =  CreateUnit( GetOwningPlayer( Caster ), 'u60E', xc, yc, bj_RADTODEG * Atan2( yt - yc, xt - xc ) )
    ...
    call TimerStart( timerer, 0.01, true, function Trig_Phoenix_arrow_timer )
...
function Trig_Phoenix_arrow_Conditions takes nothing returns boolean
    
    return GetSpellAbilityId() == 'A6BR'
endfunction

function InitTrig_Phoenix_arrow takes nothing returns nothing
    set gg_trg_Phoenix_arrow = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Phoenix_arrow, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Phoenix_arrow, Condition( function Trig_Phoenix_arrow_Conditions ) )
    call TriggerAddAction( gg_trg_Phoenix_arrow, function Trig_Phoenix_arrow_Actions )
endfunction
...
function Trig_Veer_Fire_Arrows_Timer takes nothing returns nothing

    local timer timers      = GetExpiredTimer()
    local integer i         = GetHandleId(timers)
    local group arrows      = LoadGroupHandle(udg_hash,i,1)
    local group n           = CopyGroup(arrows)
    local unit dummy        = null
    local real x
    local real y

    loop
        set  dummy  =  FirstOfGroup(n)  
        exitwhen ( dummy == null ) 
        call GroupRemoveUnit( n, dummy )
        
            if ( GetWidgetLife( dummy ) > 0.405 ) then
            
        set x = GetUnitX(dummy)+10*Cos(GetUnitFacing(dummy)*bj_DEGTORAD)
        set y = GetUnitY(dummy)+10*Sin(GetUnitFacing(dummy)*bj_DEGTORAD)
        
        call SetUnitPosition(dummy,x,y)
            else
            
        call GroupClear(arrows)
        call DestroyGroup(arrows)
        call FlushChildHashtable(udg_hash,i)
        call PauseTimer(timers)
        call DestroyTimer(timers)
...
function Trig_Veer_Fire_Arrows_Actions takes nothing returns nothing
    local unit caster      = GetTriggerUnit()
    local real x           = GetUnitX(caster)
    local real y           = GetUnitY(caster)
    local real xt          = GetSpellTargetX()
    local real yt          = GetSpellTargetY()
    local integer k        = GetUnitAbilityLevel(caster, 'A6BQ')
    local real d           = k*10+bj_RADTODEG * Atan2(yt-y,xt-x)
    local integer l        = 1
    local group arrows     = CreateGroup()
    
    local unit dummy       = null
    local timer times      = CreateTimer()
    local integer i        = GetHandleId(times)
    
     loop
            exitwhen l > 1+k*2
            set dummy = CreateUnit(GetOwningPlayer(caster),'u60F',x + 30 * Cos(d * bj_DEGTORAD),y + 30* Sin(d * bj_DEGTORAD),d)
            call UnitApplyTimedLife(dummy,'BTLF', 1)
            call SetUnitAbilityLevel(dummy,'A6BO', k)
            call GroupAddUnit(arrows, dummy)
            set d = d - 10
            set l = l + 1
            
    endloop

    call SaveGroupHandle(udg_hash,i,1,arrows)
    call TimerStart(times, 0.01, true, function Trig_Veer_Fire_Arrows_Timer)
...
function Trig_Veer_Fire_Arrows_Conditions takes nothing returns boolean
return GetSpellAbilityId() == 'A6BQ'
endfunction

function InitTrig_Veer_Fire_Arrows takes nothing returns nothing
    set gg_trg_Veer_Fire_Arrows = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Veer_Fire_Arrows, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Veer_Fire_Arrows, Condition( function Trig_Veer_Fire_Arrows_Conditions ) )
    call TriggerAddAction( gg_trg_Veer_Fire_Arrows, function Trig_Veer_Fire_Arrows_Actions )
endfunction

//===========================================================================


function Trig_Wave_Of_Fire_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A64A'
endfunction

function Trig_Wave_Of_Fire_Actions takes nothing returns nothing
    local unit caster
    local location caster_point
    local location dummy_point
    set caster = GetSpellAbilityUnit()
    set caster_point = GetUnitLoc(caster)
    set bj_forLoopAIndex = 1
    ...
    loop
        exitwhen bj_forLoopAIndex > bj_forLoopAIndexEnd
        set dummy_point = PolarProjectionBJ(caster_point, 1.00, ( 22.50 * I2R(GetForLoopIndexA()) ))
        call CreateNUnitsAtLoc( 1, 'h604', GetOwningPlayer(caster), caster_point, bj_UNIT_FACING )
        call SetUnitAbilityLevelSwapped( 'A661', GetLastCreatedUnit(), GetUnitAbilityLevelSwapped('A64A', caster) )
        call ShowUnitHide( GetLastCreatedUnit() )
        call UnitApplyTimedLifeBJ( 1.00, 'BTLF', GetLastCreatedUnit() )
        call IssuePointOrderLocBJ( GetLastCreatedUnit(), "breathoffire", dummy_point )
        call RemoveLocation(dummy_point)
        set bj_forLoopAIndex = bj_forLoopAIndex + 1
    endloop
    call RemoveLocation(caster_point)
    set caster = null
endfunction

function InitTrig_Wave_Of_Fire takes nothing returns nothing
    set gg_trg_Wave_Of_Fire = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Wave_Of_Fire, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Wave_Of_Fire, Condition( function Trig_Wave_Of_Fire_Conditions ) )
    call TriggerAddAction( gg_trg_Wave_Of_Fire, function Trig_Wave_Of_Fire_Actions )
endfunction
//===========================================================================
...
// Oceanic_Flow
//===========================================================================
    function Trig_Oceanic_Flow_Conditions takes nothing returns boolean
        return GetLearnedSkillBJ() == 'A66W'
    endfunction

    function Trig_Oceanic_Flow_Actions takes nothing returns nothing
        local unit caster
        local integer spell_level
        set caster = GetLearningUnit()
        set spell_level = GetUnitAbilityLevelSwapped('A66W', caster)
        if ( spell_level == 1 ) then
            call UnitAddAbilityBJ( 'A688', caster )
            call UnitAddAbilityBJ( 'A6BX', caster )
        else
            if ( spell_level == 2 ) then
                call UnitAddAbilityBJ( 'A6BV', caster )
                call UnitAddAbilityBJ( 'A68I', caster )
            else
                if ( spell_level == 3 ) then
                    call UnitAddAbilityBJ( 'A6BW', caster )
                    call UnitAddAbilityBJ( 'A6BY', caster )
                else
                    call DoNothing(  )
                endif
            endif
...
//===========================================================================
function InitTrig_Oceanic_Flow takes nothing returns nothing
    set gg_trg_Oceanic_Flow = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Oceanic_Flow, EVENT_PLAYER_HERO_SKILL )
    call TriggerAddCondition( gg_trg_Oceanic_Flow, Condition( function Trig_Oceanic_Flow_Conditions ) )
    call TriggerAddAction( gg_trg_Oceanic_Flow, function Trig_Oceanic_Flow_Actions )
endfunction
//===========================================================================
// Water_Geyser
//===========================================================================
function Trig_Water_Geyser_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A670'
endfunction


function Trig_Water_Geyser_Func006A takes nothing returns nothing
    local unit caster = GetSpellAbilityUnit()
    local unit target = GetEnumUnit()
    local integer handle_id = GetHandleId(caster)
    local player caster_player = GetOwningPlayer(caster)
    local real damage = LoadReal(udg_hash,handle_id,2)

    if ( ( IsUnitType(GetEnumUnit(), UNIT_TYPE_STRUCTURE) == false ) and ( IsUnitType(GetEnumUnit(), UNIT_TYPE_FLYING) == false ) and ( IsUnitType(GetEnumUnit(), UNIT_TYPE_MAGIC_IMMUNE) == false ) and ( IsUnitAliveBJ(GetEnumUnit()) == true ) and ( IsUnitEnemy(GetEnumUnit(), caster_player) == true ) ) then
        
        call UnitDamageTargetBJ( caster, GetEnumUnit(), damage, ATTACK_TYPE_NORMAL, DAMAGE_TYPE_COLD )
...
function Trig_Water_Geyser_Actions takes nothing returns nothing
    local unit caster                       = GetSpellAbilityUnit()
    local location target_point             = GetSpellTargetLoc()
    local real range                        = 250.00
    local integer spell_level               = GetUnitAbilityLevelSwapped('A670', caster)
    local real damage                       = ( 85.00 + ( 50.00 * I2R(spell_level) ) )
    local integer handle_id                 = GetHandleId(caster)

    call SaveUnitHandle(udg_hash,handle_id,1,caster)
    call SaveReal(udg_hash,handle_id,2,damage)

    call ForGroupBJ( GetUnitsInRangeOfLocAll(range, target_point), function Trig_Water_Geyser_Func006A )
    // -
    call AddSpecialEffectLocBJ( target_point, "Effect_WaterBlast.mdx" )
    call DestroyEffectBJ( GetLastCreatedEffectBJ() )
    // -
...
//===========================================================================
function InitTrig_Water_Geyser takes nothing returns nothing
    set gg_trg_Water_Geyser = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Water_Geyser, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Water_Geyser, Condition( function Trig_Water_Geyser_Conditions ) )
    call TriggerAddAction( gg_trg_Water_Geyser, function Trig_Water_Geyser_Actions )
endfunction
//***************************************************************************
//*
//*  Dark Orc Spells 
//*
//***************************************************************************
// ВОЖДЬ ОРДЫ - ГРОК-ТАР / КРОВЬ ДЕМОНА
//===========================================================================
function Trig_Blood_of_Demon_Conditions takes nothing returns boolean
    return GetLearnedSkillBJ() == 'A65G'
endfunction

function Trig_Blood_of_Demon_Actions takes nothing returns nothing
    local unit caster
    local integer spell_level

    set caster = GetLearningUnit()
    set spell_level = GetUnitAbilityLevelSwapped('A65G', caster)
    if ( spell_level == 1 ) then
        call UnitAddAbilityBJ( 'A681', caster )
        call UnitAddAbilityBJ( 'A68D', caster )
    else
        if ( spell_level == 2 ) then
            call UnitAddAbilityBJ( 'A68J', caster )
            call UnitAddAbilityBJ( 'A687', caster )
        else
            if ( spell_level == 3 ) then
                call UnitAddAbilityBJ( 'A6BT', caster )
                call UnitAddAbilityBJ( 'A689', caster )
            else
                call DoNothing( )
            endif
        endif
...
//===========================================================================
function InitTrig_Blood_of_Demon takes nothing returns nothing
    set gg_trg_Blood_of_Demon = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Blood_of_Demon, EVENT_PLAYER_HERO_SKILL )
    call TriggerAddCondition( gg_trg_Blood_of_Demon, Condition( function Trig_Blood_of_Demon_Conditions ) )
    call TriggerAddAction( gg_trg_Blood_of_Demon, function Trig_Blood_of_Demon_Actions )
endfunction
//===========================================================================
//
//  ПОВЕЛИТЕЛЬ ВОЙНЫ
...
//  ЯРОСТЬ ВОЙНЫ
//===========================================================================
function Trig_Rage_of_War_RemoveBuff takes nothing returns nothing
    local timer t           = GetExpiredTimer( )
    local integer i         = GetHandleId( t )
    local unit caster       = LoadUnitHandle( udg_hash, i, 1 )

    call UnitRemoveAbilityBJ( 'A67I', caster )
    call UnitRemoveAbilityBJ( 'A67X', caster )
...
function Trig_Rage_of_War_Conditions takes nothing returns boolean

    return GetSpellAbilityId() == 'A65U'
endfunction

function Trig_Rage_of_War_Actions takes nothing returns nothing
    ...
    call SaveUnitHandle( udg_hash, i, 1, caster )

    call UnitAddAbilityBJ( 'A67I', caster )
    call UnitAddAbilityBJ( 'A67X', caster )
    call TimerStart( t, 30, false, function Trig_Rage_of_War_RemoveBuff )

    set caster  = null
    set t       = null
endfunction

function InitTrig_Rage_of_War takes nothing returns nothing
    set gg_trg_Rage_of_War = CreateTrigger(  )
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Rage_of_War, EVENT_PLAYER_UNIT_SPELL_EFFECT )
    call TriggerAddCondition( gg_trg_Rage_of_War, Condition( function Trig_Rage_of_War_Conditions ) )
    call TriggerAddAction( gg_trg_Rage_of_War, function Trig_Rage_of_War_Actions )
endfunction
//===========================================================================
//  ЖАЖДА КРОВИ
//===========================================================================
...
function RemoveBuff takes nothing returns nothing
    ...
    call UnitRemoveAbilityBJ( 'A67Q', caster )
    call UnitRemoveAbilityBJ( 'A67S', caster )
    call UnitRemoveAbilityBJ( 'A67U', caster )
    call UnitRemoveAbilityBJ( 'A684', caster )
    call UnitRemoveAbilityBJ( 'A685', caster )
    call UnitRemoveAbilityBJ( 'A686', caster ) 
...
function Trig_Bloodlust_Actions takes nothing returns nothing
    ...
    set spell_level = GetUnitAbilityLevelSwapped('A65H', caster)

    if ( spell_level == 1 ) then
        call UnitAddAbilityBJ( 'A67Q', caster )
        call UnitAddAbilityBJ( 'A684', caster )
        call SaveUnitHandle(udg_hash,h,1,caster)
        call TimerStart(t,15.00,false, function RemoveBuff)
    else
        if ( spell_level == 2 ) then
            call UnitAddAbilityBJ( 'A67S', caster )
            call UnitAddAbilityBJ( 'A685', caster )
            call SaveUnitHandle(udg_hash,h,1,caster)
            call TimerStart(t,15.00,false, function RemoveBuff)

        else
            if ( spell_level == 3 ) then
                call UnitAddAbilityBJ( 'A67U', caster )
                call UnitAddAbilityBJ( 'A686', caster )
                call SaveUnitHandle(udg_hash,h,1,caster)
                call TimerStart(t,15.00,false, function RemoveBuff)
            else
                call DoNothing()
...
function InitTrig_Bloodlust takes nothing returns nothing
    ...
    call TriggerRegisterAnyUnitEventBJ( gg_trg_Bloodlust, EVENT_PLAYER_UNIT_SPELL_EFFECT )
...
function MeleeStartingHeroLimit takes nothing returns nothing
    ...
    loop
		...
		call ReducePlayerTechMaxAllowed(Player(index), 'H600', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H601', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H602', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H603', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'O00A', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H607', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'H600', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'E600', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H601', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'H63F', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H63G', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H63E', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'E60Z', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'O61Q', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O61R', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O61T', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'O61S', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O61U', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O61W', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O644', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'H636', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H637', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H638', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O63Z', bj_MELEE_HERO_TYPE_LIMIT)

        call ReducePlayerTechMaxAllowed(Player(index), 'H634', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'H635', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'U60B', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'U603', bj_MELEE_HERO_TYPE_LIMIT)
        
        call ReducePlayerTechMaxAllowed(Player(index), 'N60X', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'E60A', bj_MELEE_HERO_TYPE_LIMIT)
        ...
        call ReducePlayerTechMaxAllowed(Player(index), 'H64C', bj_MELEE_HERO_TYPE_LIMIT)
		call ReducePlayerTechMaxAllowed(Player(index), 'H64P', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'E611', bj_MELEE_HERO_TYPE_LIMIT)

        //===========================================================================
        // ** Герои дренеев **
        call ReducePlayerTechMaxAllowed(Player(index), 'N632', bj_MELEE_HERO_TYPE_LIMIT)

        call ReducePlayerTechMaxAllowed(Player(index), 'H641', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'O643', bj_MELEE_HERO_TYPE_LIMIT)
        ...
        // ** Герои Даларана **
        // Морозная лучница
        call ReducePlayerTechMaxAllowed(Player(index), 'H63Q', bj_MELEE_HERO_TYPE_LIMIT)
        // Архимаг
        call ReducePlayerTechMaxAllowed(Player(index), 'H63R', bj_MELEE_HERO_TYPE_LIMIT)
        // Чародей
        call ReducePlayerTechMaxAllowed(Player(index), 'H63S', bj_MELEE_HERO_TYPE_LIMIT)
        // Аспект Магии
        call ReducePlayerTechMaxAllowed(Player(index), 'H63T', bj_MELEE_HERO_TYPE_LIMIT)
        // Чародейка
        call ReducePlayerTechMaxAllowed(Player(index), 'H64K', bj_MELEE_HERO_TYPE_LIMIT)
        ...
        // ** Герои Ящеров **
        // Странник
        call ReducePlayerTechMaxAllowed(Player(index), 'O612', bj_MELEE_HERO_TYPE_LIMIT)
        // Высшая ведьма 
        call ReducePlayerTechMaxAllowed(Player(index), 'H60Y', bj_MELEE_HERO_TYPE_LIMIT)
        // Гумунгазавр 
        call ReducePlayerTechMaxAllowed(Player(index), 'O62G', bj_MELEE_HERO_TYPE_LIMIT)
        // Вождь Острова Клыка 
        call ReducePlayerTechMaxAllowed(Player(index), 'O611', bj_MELEE_HERO_TYPE_LIMIT)
        ...
        // ** Герои Нерубов **
        call ReducePlayerTechMaxAllowed(Player(index), 'U604', bj_MELEE_HERO_TYPE_LIMIT)
        call ReducePlayerTechMaxAllowed(Player(index), 'U605', bj_MELEE_HERO_TYPE_LIMIT)
        ...
        // ** Герои таверны **
        // Морлок-чернокнижник
        call ReducePlayerTechMaxAllowed(Player(index), 'E615', bj_MELEE_HERO_TYPE_LIMIT)
        // Адмирал
        ...
        // ** Новые герои **
        // Вестница смерти
        call ReducePlayerTechMaxAllowed(Player(index), 'U61J', bj_MELEE_HERO_TYPE_LIMIT)
        // Морской змей
        ...
        // ** Герои Сатиров **
        // Оверлорд
        call ReducePlayerTechMaxAllowed(Player(index), 'N655', bj_MELEE_HERO_TYPE_LIMIT)
        // Старейшина сатиров
        call ReducePlayerTechMaxAllowed(Player(index), 'O64H', bj_MELEE_HERO_TYPE_LIMIT)
        // Проклятый дух леса
        call ReducePlayerTechMaxAllowed(Player(index), 'E61D', bj_MELEE_HERO_TYPE_LIMIT)
        // Теневой ассасин
        call ReducePlayerTechMaxAllowed(Player(index), 'O63E', bj_MELEE_HERO_TYPE_LIMIT)
...
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsNaga takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'n61T', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'n61Q', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'n61Q', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n61Q', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for Dwarven Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsDwarven takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h60S', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'h605', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'h605', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h605', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for DarkOrc Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsDarkOrc takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'o60O', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'n617', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'n617', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n617', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for IceTrolls Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsIceTrolls takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'o605', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o60H', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o60H', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60H', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for BloodElfs Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsBloodElfs takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h61Q', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'n60P', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60P', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60P', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60P', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60P', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'n60p', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60p', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60p', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60p', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60p', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'H636', 'H637', 'H638', 'H64P', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for ForestGuard Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsForestGuard takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h61O', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'h613', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'h613', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h613', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'N60X', 'E60A', 'E60B', 'H64Q', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for Forsaken Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsForsaken takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h61R', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'h611', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'h611', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h611', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'H634', 'H635', 'U60B', 'U603', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for Goblin Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsGoblin takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'o610', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o60U', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o60U', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o60U', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for Lizards Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsLizards takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'o623', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o616', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o616', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o616', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'H60Y', 'O612', 'O62G', 'O611', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Стартовые юниты для Игрока - Даларан
//   - 1 Школа магов в позиции старта игрока
//   - 5 Работников, в позиции между статом игрока и золотой шахтой
//
//	* * * * *
//
// Starting Units for Dalaran Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsKirintor takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h63H', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'h63J', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'h63J', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'h63J', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'H63T', 'H63S', 'H63R', 'H63Q', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for HighElf Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsHighElf takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'h61P', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'n60Q', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'n60Q', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n60Q', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'H639', 'H63A', 'H64C', 'E611', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for Gnoll Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsGnoll takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'o62H', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o634', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o634', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o634', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
...
//===========================================================================
// Starting Units for Demons Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsDemons takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'u60H', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'n63M', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'n63M', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'n63M', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'U60G', 'N63G', 'N63H', 'U60S', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for Nerubian Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsNerubian takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'u60W', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o64E', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o64E', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o64E', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'U604', 'U605', 'H64U', 'N63F', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
//===========================================================================
// Starting Units for Satyrs Players
//   - 1 Great Hall, placed at start location
//   - 5 Peons, placed between start location and nearest gold mine
//
function MeleeStartingUnitsSatyrs takes player whichPlayer, location startLoc, boolean doHeroes, boolean doCamera, boolean doPreload returns nothing
    local boolean  useRandomHero = IsMapFlagSet(MAP_RANDOM_HERO)
    local real     unitSpacing   = 64.00
    local unit     nearestMine
    local location nearMineLoc
    ...
    if (nearestMine != null) then
        // Spawn Great Hall at the start location.
        call CreateUnitAtLoc(whichPlayer, 'e60P', startLoc, bj_UNIT_FACING)
        
        // Spawn Peons near the mine.
        set nearMineLoc = MeleeGetProjectedLoc(GetUnitLoc(nearestMine), startLoc, 320, 0)
        set peonX = GetLocationX(nearMineLoc)
        set peonY = GetLocationY(nearMineLoc)
        call CreateUnit(whichPlayer, 'o633', peonX + 0.00 * unitSpacing, peonY + 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX + 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX - 1.00 * unitSpacing, peonY + 0.15 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX + 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX - 0.60 * unitSpacing, peonY - 1.00 * unitSpacing, bj_UNIT_FACING)
        ...
        set peonX = GetLocationX(startLoc)
        set peonY = GetLocationY(startLoc) - 224.00
        call CreateUnit(whichPlayer, 'o633', peonX + 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX + 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX + 0.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX - 1.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
        call CreateUnit(whichPlayer, 'o633', peonX - 2.00 * unitSpacing, peonY + 0.00 * unitSpacing, bj_UNIT_FACING)
    ...
    if (doHeroes) then
        ...
        // Otherwise, give them a "free hero" token.
        if useRandomHero then
            call MeleeRandomHeroLoc(whichPlayer, 'O64H', 'E61D', 'N655', 'O63E', heroLoc)
        else
            call SetPlayerState(whichPlayer, PLAYER_STATE_RESOURCE_HERO_TOKENS, bj_MELEE_STARTING_HERO_TOKENS)
        endif
    endif
...
function MeleeStartingUnits takes nothing returns nothing
    ...
    loop
        ...
        if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
			...
			//call BJDebugMsg( ( "Player"+I2S(index)+" RaceIndex:" + I2S(MME_R2I(indexRace)) ) )
            // Create initial race-specific starting units
            if (indexRace == MME_RACE_HUMAN) then
                call MeleeStartingUnitsHuman(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_ORC) then
                call MeleeStartingUnitsOrc(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_UNDEAD) then
                call MeleeStartingUnitsUndead(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_NIGHTELF) then
                call MeleeStartingUnitsNightElf(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_NAGA) then
                call MeleeStartingUnitsNaga(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_DWARVEN) then
                call MeleeStartingUnitsDwarven(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_DARKORC) then
                call MeleeStartingUnitsDarkOrc(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_ICETROLLS) then
                call MeleeStartingUnitsIceTrolls(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_BLOODELFS) then
                call MeleeStartingUnitsBloodElfs(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_FORESTGUARD) then
                call MeleeStartingUnitsForestGuard(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_FORSAKEN) then
                call MeleeStartingUnitsForsaken(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_GOBLIN) then
                call MeleeStartingUnitsGoblin(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_LIZARDS) then
                call MeleeStartingUnitsLizards(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_KIRINTOR) then
                call MeleeStartingUnitsKirintor(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_HIGHELF) then
                call MeleeStartingUnitsHighElf(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_GNOLL) then
                call MeleeStartingUnitsGnoll(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_DEMONS) then
                call MeleeStartingUnitsDemons(indexPlayer, indexStartLoc, true, true, true)
			elseif (indexRace == MME_RACE_NERUBIAN) then
                call MeleeStartingUnitsNerubian(indexPlayer, indexStartLoc, true, true, true)
            elseif (indexRace == MME_RACE_SATYRS) then
                call MeleeStartingUnitsSatyrs(indexPlayer, indexStartLoc, true, true, true)
            else
    ...
    loop
        ...
        if (GetPlayerSlotState(indexPlayer) == PLAYER_SLOT_STATE_PLAYING) then
            ...
            if (GetPlayerController(indexPlayer) == MAP_CONTROL_COMPUTER) then
				...
				elseif (indexRace == MME_RACE_DWARVEN) then
                    call PickMeleeAI(indexPlayer, "dwarven.ai", null, null)
				elseif (indexRace == MME_RACE_DARKORC) then
                    call PickMeleeAI(indexPlayer, "darkorc.ai", null, null)
				elseif (indexRace == MME_RACE_ICETROLLS) then
                    call PickMeleeAI(indexPlayer, "icetrolls.ai", null, null)
				elseif (indexRace == MME_RACE_BLOODELFS) then
                    call PickMeleeAI(indexPlayer, "bloodelfs.ai", null, null)
                elseif (indexRace == MME_RACE_FORESTGUARD) then
                    call PickMeleeAI(indexPlayer, "forestguard.ai", null, null)
                elseif (indexRace == MME_RACE_FORSAKEN) then
                    call PickMeleeAI(indexPlayer, "forsaken.ai", null, null)
				elseif (indexRace == MME_RACE_GOBLIN) then
                    call PickMeleeAI(indexPlayer, "goblin.ai", null, null)
                elseif (indexRace == MME_RACE_LIZARDS) then
                    call PickMeleeAI(indexPlayer, "lizards.ai", null, null)
                elseif (indexRace == MME_RACE_KIRINTOR) then
                    call PickMeleeAI(indexPlayer, "kirintor.ai", null, null)
                elseif (indexRace == MME_RACE_HIGHELF) then
                    call PickMeleeAI(indexPlayer, "highelf.ai", null, null)
                elseif (indexRace == MME_RACE_GNOLL) then
                    call PickMeleeAI(indexPlayer, "gnoll.ai", null, null)
				elseif (indexRace == MME_RACE_DEMONS) then
                    call PickMeleeAI(indexPlayer, "demons.ai", null, null)
				elseif (indexRace == MME_RACE_NERUBIAN) then
                    call PickMeleeAI(indexPlayer, "nerubian.ai", null, null)
                elseif (indexRace == MME_RACE_SATYRS) then
                    call PickMeleeAI(indexPlayer, "satyrs.ai", null, null)
                else
                    // Unrecognized race.
                endif
...
function MeleeGetAllyKeyStructureCount takes player whichPlayer returns integer
    ...
    loop
        ...
        if (PlayersAreCoAllied(whichPlayer, indexPlayer)) then
			...
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h60S'), true, true)
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('n61T'), true, true)
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h61Q'), true, true)
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o60O'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h61R'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h61O'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o605'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o610'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h63H'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('h61P'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o623'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o62H'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('o603'), true, true)
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('u60H'), true, true)
			set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('u60W'), true, true)
            set keyStructs = keyStructs + GetPlayerTypedUnitCount(indexPlayer, UnitId2StringBJ('e60P'), true, true)
        endif
...
function InitSummonableCaps takes nothing returns nothing
    ...
    loop
        ...
        // Улучшение Парового танка Dwarven
        if (not GetPlayerTechResearched(Player(index), 'R60E', true)) then
            call SetPlayerTechMaxAllowed(Player(index), 'h60U', 0)
        endif

        // Улучшение бронзового морского дракона
        if (not GetPlayerTechResearched(Player(index), 'R63G', true)) then
            call SetPlayerTechMaxAllowed(Player(index), 'n622', 0)
        endif

        // Улучшение ящеров-охотников
...
//==============================MM Engine====================================
//===========================================================================
function InitMM_Engine takes nothing returns nothing
	// Custom scripts 3

    // Инициализация / запуск новых триггеров. 
    // ШАГ 6
    call InitTrig_Camera_Zoom()
    call InitTrig_Chat_Name()
    call Trig_RealCamera()
    call InitTrig_StackItems()
    call InitTrig_Hash()

    call InitTrig_SniperScope()
	call InitTrig_Stunning_Shock()
    call InitTrig_Lightning_Storm()
    call InitTrig_Rune_of_Health()
    call InitTrig_Rune_of_Power()
    call InitTrig_Rune_of_Speed()

    call InitTrig_Phoenix_arrow()
    call InitTrig_Veer_Fire_Arrows()
    call InitTrig_Wave_Of_Fire()

    call InitTrig_Rage_of_War()
    call InitTrig_Bloodlust()
    call InitTrig_Blood_of_Demon()


 call InitTrig_TrigerNerub(  )
    call InitTrig_TrigerNerub2(  )

    call InitTrig_svetslepot(  )
    call InitTrig_samobaff(  )
    call InitTrig_obnulbaffa(  )
    call InitTrig_metkadamik(  )
    call InitTrig_silaStihii(  )
    call InitTrig_silaStihi2(  )
    call InitTrig_silaStihi3(  )
    call InitTrig_prizivelema(  )
    call InitTrig_arhimagosila(  )
    call InitTrig_prokelem(  )
    ...
    call InitTrig_Oceanic_Flow()
    call InitTrig_Water_Geyser()
    call InitTrig_LizardmanRiderAnimationDebug()
    call InitTrig_StartedCameraSet()



    // Принудительный запуск триггера с камерой 
    call TriggerExecute( gg_trg_StartedCameraSet )
endfunction
